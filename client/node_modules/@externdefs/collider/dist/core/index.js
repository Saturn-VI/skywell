import { BATCH_REGEX, BINDING_REGEX, HELP_COMMAND_INDEX, HELP_REGEX, IS_DEBUG, NodeType, OPTION_REGEX, SpecialToken, } from './constants.js';
...
export function makeAnyOfMachine(inputs) {
...
export function injectNode(machine, node) {
    machine.nodes.push(node);
    return machine.nodes.length - 1;
}
export function simplifyMachine(input) {
...
export function debugMachine(machine, { prefix = `` } = {}) {
...
export function runMachineInternal(machine, input, partial = false) {
    ...
    for (let t = 0; t < tokens.length; ++t) {
            ...
            if (!partial || t < tokens.length - 1 || hasExactMatch) {
                if (hasExactMatch) {
                    ...
                    for (const { to, reducer } of transitions) {
                        nextBranches.push({
                            node: to,
                            state: typeof reducer !== `undefined`
                                ? execute(reducers, reducer, state, segment, segmentIndex)
                                : state,
                        });
        ...
        if (nextBranches.length === 0) {
            throw new errors.UnknownSyntaxError(input, branches
                .filter(({ node }) => {
...
export function selectBestState(input, states) {
    ...
    const requiredOptionsSetStates = terminalStates.filter((state) => state.selectedIndex === HELP_COMMAND_INDEX ||
        state.requiredOptions.every((names) => names.some((name) => state.options.find((opt) => opt.name === name))));
...
export function aggregateHelpStates(states) {
    const notHelps = [];
    const helps = [];
...
export function cloneTransition(input, offset = 0) {
...
export function cloneNode(input, offset = 0) {
...
export function registerDynamic(machine, from, test, to, reducer) {
    machine.nodes[from].dynamics.push([
        test,
        { to, reducer: reducer },
    ]);
}
export function registerShortcut(machine, from, to, reducer) {
    machine.nodes[from].shortcuts.push({
        to,
        reducer: reducer,
    });
}
export function registerStatic(machine, from, test, to, reducer) {
...
export function execute(store, callback, state, segment, segmentIndex) {
...
export const tests = {
    ...
    isOption: (state, segment, _segmentIndex, name) => {
        return !state.ignoreOptions && segment === name;
    },
    isBatchOption: (state, segment, _segmentIndex, names) => {
        return (!state.ignoreOptions &&
            BATCH_REGEX.test(segment) &&
            [...segment.slice(1)].every((name) => names.has(`-${name}`)));
    },
    isBoundOption: (state, segment, _segmentIndex, names, options) => {
        const optionParsing = segment.match(BINDING_REGEX);
        return (!state.ignoreOptions &&
            !!optionParsing &&
            OPTION_REGEX.test(optionParsing[1]) &&
            names.has(optionParsing[1]) &&
    ...
    isNegatedOption: (state, segment, _segmentIndex, name) => {
        return !state.ignoreOptions && segment === `--no-${name.slice(2)}`;
    },
    isHelp: (state, segment) => {
        return !state.ignoreOptions && HELP_REGEX.test(segment);
    },
    isUnsupportedOption: (state, segment, _segmentIndex, names) => {
...
export const reducers = {
    setCandidateState: (state, _segment, _segmentIndex, candidateState) => {
        return { ...state, ...candidateState };
    },
    setSelectedIndex: (state, _segment, _segmentIndex, index) => {
    ...
    useHelp: (state, segment, _segmentIndex, command) => {
    ...
    setError: (state, segment, _segmentIndex, errorMessage) => {
    ...
    setOptionArityError: (state, _segment) => {
        const lastOption = state.options[state.options.length - 1];
...
export class CommandBuilder {
    cliIndex;
    cliOpts;
    allOptionNames = new Map();
    arity = { leading: [], trailing: [], extra: [], proxy: false };
    options = [];
    paths = [];
    context;
    ...
    compile() {
        ...
        registerStatic(machine, NodeType.InitialNode, SpecialToken.StartOfInput, firstNode, [
            `setCandidateState`,
            { candidateUsage, requiredOptions },
        ]);
        ...
        for (const path of paths) {
            ...
            for (let t = 0; t < path.length; ++t) {
                ...
                if (t + 1 < path.length) {
                    ...
                    registerStatic(machine, helpNode, SpecialToken.EndOfInput, NodeType.SuccessNode, [
                        `setSelectedIndex`,
                        HELP_COMMAND_INDEX,
                    ]);
                }
            ...
            if (this.arity.leading.length > 0) {
                registerStatic(machine, lastPathNode, SpecialToken.EndOfInput, NodeType.ErrorNode, [
                    `setError`,
                    `Not enough positional arguments`,
                ]);
                registerStatic(machine, lastPathNode, SpecialToken.EndOfPartialInput, NodeType.SuccessNode, [
                    `setSelectedIndex`,
                    this.cliIndex,
                ]);
            }
            ...
            for (let t = 0; t < this.arity.leading.length; ++t) {
                ...
                if (this.arity.trailing.length > 0 || t + 1 !== this.arity.leading.length) {
                    registerStatic(machine, nextLeadingNode, SpecialToken.EndOfInput, NodeType.ErrorNode, [
                        `setError`,
                        `Not enough positional arguments`,
                    ]);
                    registerStatic(machine, nextLeadingNode, SpecialToken.EndOfPartialInput, NodeType.SuccessNode, [
                        `setSelectedIndex`,
                        this.cliIndex,
                    ]);
                }
            ...
            if (this.arity.trailing.length > 0) {
                registerStatic(machine, lastExtraNode, SpecialToken.EndOfInput, NodeType.ErrorNode, [
                    `setError`,
                    `Not enough positional arguments`,
                ]);
                registerStatic(machine, lastExtraNode, SpecialToken.EndOfPartialInput, NodeType.SuccessNode, [
                    `setSelectedIndex`,
                    this.cliIndex,
                ]);
            }
            ...
            for (let t = 0; t < this.arity.trailing.length; ++t) {
                ...
                if (t + 1 < this.arity.trailing.length) {
                    registerStatic(machine, nextTrailingNode, SpecialToken.EndOfInput, NodeType.ErrorNode, [
                        `setError`,
                        `Not enough positional arguments`,
                    ]);
                    registerStatic(machine, nextTrailingNode, SpecialToken.EndOfPartialInput, NodeType.SuccessNode, [
                        `setSelectedIndex`,
                        this.cliIndex,
                    ]);
                }
            ...
            registerDynamic(machine, lastTrailingNode, positionalArgument, NodeType.ErrorNode, [
                `setError`,
                `Extraneous positional argument`,
            ]);
            registerStatic(machine, lastTrailingNode, SpecialToken.EndOfInput, NodeType.SuccessNode, [
                `setSelectedIndex`,
                this.cliIndex,
            ]);
            registerStatic(machine, lastTrailingNode, SpecialToken.EndOfPartialInput, NodeType.SuccessNode, [
                `setSelectedIndex`,
                this.cliIndex,
            ]);
        }
    ...
    registerOptions(machine, node) {
        ...
        registerDynamic(machine, node, [`isBatchOption`, this.allOptionNames], node, [
            `pushBatch`,
            this.allOptionNames,
        ]);
        registerDynamic(machine, node, [`isBoundOption`, this.allOptionNames, this.options], node, `pushBound`);
        registerDynamic(machine, node, [`isUnsupportedOption`, this.allOptionNames], NodeType.ErrorNode, [
            `setError`,
            `Unsupported option name`,
        ]);
        registerDynamic(machine, node, [`isInvalidOption`], NodeType.ErrorNode, [
            `setError`,
            `Invalid option name`,
        ]);
            ...
            if (option.arity === 0) {
                for (const name of option.nameSet) {
                    ...
                    if (name.startsWith(`--`) && !name.startsWith(`--no-`)) {
                        registerDynamic(machine, node, [`isNegatedOption`, name], node, [
                            `pushFalse`,
                ...
                for (const name of option.nameSet) {
                    registerDynamic(machine, node, [`isOption`, name], lastNode, [
                        `pushUndefined`,
                        option.preferredName,
                    ]);
                }
